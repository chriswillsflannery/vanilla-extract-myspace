...
const resolvedFileScopeModule = path.dirname(require.resolve('@vanilla-extract/css/fileScope/package.json'));
const resolvedCoreModule = path.dirname(require.resolve('@vanilla-extract/css/package.json'));
      ...
      compiler.hooks.compilation.tap(pluginName, compilation => {
        compilation.hooks.afterOptimizeModules.tap(pluginName, modules => {
          for (const module of modules) {
            ...
            if ( // Use the presence of the `fileScope` module as the indicator that the runtime has been loaded
            (_module$context = module.context) !== null && _module$context !== void 0 && _module$context.startsWith(resolvedFileScopeModule)) {
              const dependentResources = new Set();

              if (compat.isWebpack5) {
                for (const con of compilation.moduleGraph.getIncomingConnections(module)) {
              ...
              let errorMessage = chalk.red(`VanillaExtractPlugin: Styles detected outside of '.css.(ts/js)' files. These styles cannot be statically extracted.`);

              if (dependentResources.size > 0) {
                errorMessage += '\n\nOffending files:\n';
                errorMessage += Array.from(dependentResources).map(res => `- ${chalk.yellow(path.relative(compiler.context, res))}`).join('\n');
              }
